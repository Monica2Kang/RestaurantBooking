...
protected:
	void SetUp() override {
		...
		SUNDAY_SCHEDULE = getTime(2021, 3, 28, 17, 0);
		MONDAY_SCHEDULE = getTime(2024, 6, 3, 17, 0);
...
public:
	...
	Customer CUSTOMER_WITH_MAIL = { "Fake Name", "010-1234-56780", "test@test.com" };
...
TEST_F(BookingItem, 예약은정시에만가능하다정시가아닌경우예약불가) {
	//arrange
	Schedule* schedule = new Schedule{ NON_ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER };
...
TEST_F(BookingItem, 시간대별인원제한이있다같은시간대에Capacity초과할경우예외발생) {
	...
	try {
		...
		FAIL(); //Exception 발생할 예정이기에, 이 함수에 도달하지 않음
	}
...
TEST_F(BookingItem, 예약완료시SMS는무조건발송) {
	...
	//assert
	EXPECT_EQ(true, testableSmsSender.isSendMothedIsCalled());
...
TEST_F(BookingItem, 현재날짜가일요일인경우예약불가예외처리) {
	//arrange 
	BookingScheduler* bookingScheduler = new TestableBookingScheduler{ CAPACITY_PER_HOUR, SUNDAY_SCHEDULE };
...
TEST_F(BookingItem, 현재날짜가일요일이아닌경우예약가능) {
	//arrange 
	BookingScheduler* bookingScheduler = new TestableBookingScheduler{ CAPACITY_PER_HOUR, MONDAY_SCHEDULE };
